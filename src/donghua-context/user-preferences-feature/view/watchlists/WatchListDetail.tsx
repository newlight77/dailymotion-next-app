...
export const WatchListDetail: React.FC<Props> = ({ listId, className }) => {
  ...
  const listItems = (watchLists.items || [])
    ...
    .map(item => ({
      ...item,
      orderScore: item.anime ? orderScore(item.anime, item.anime ? ((item.anime as unknown as AnimeType & { rating?: { average: number } }).rating?.average ?? 0) : 0) : Number.NEGATIVE_INFINITY,
    }))
